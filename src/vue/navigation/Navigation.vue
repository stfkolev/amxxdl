<template>
    <section class="navigation">
        ...
        <!-- More menu links -->
        <nav-item title="Настройки" to="/settings"/>
        <nav-item title="За приложението" to="/about"/>
    ...
    export default {
        ...
        computed: {
            ...
            downloadSubs() {
                ...
                return [
                    {text: 'Всички', value: downloads.length},
                    {text: 'Активни', value: downloads.filter(v => v.status === 'progress').length},
                    {text: 'Завършени', value: downloads.filter(v => v.status === 'finish').length},
                    {text: 'Провалени', value: downloads.filter(v => v.status === 'errored').length},
                    {text: 'Спрени', value: downloads.filter(v => v.status === 'cancelled').length}
                ];
            }
        }
    };
