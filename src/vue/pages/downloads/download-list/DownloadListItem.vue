    ...
    <div :data-card-size="cardSize" class="download-item">
        ...
        <div class="progress">
            ...
            <div class="info-text">
                ...
                <b v-if="download.status === 'finish'"> - Отне {{ downloadDuration }}</b>
            </div>
        ...
        <div class="actions">
            ...
            <template v-if="['progress', 'paused'].includes(download.status)">
                <button class="action-red" @click="cancelDownload">Прекъсни</button>
                <button class="action-orange" @click="pauseOrResumeDownload">{{ download.status === 'paused' ? 'Продължи' : 'Пауза' }}</button>
            </template>
            ...
            <template v-if="['errored', 'cancelled'].includes(download.status)">
                <button class="action-blue" @click="retryDownload">Опитай отново</button>
            </template>
            ...
            statusText() {
                switch (this.download.status) {
                    ...
                    case 'finish':
                        return 'Завършено!';
                    case 'convert':
                        return 'Преобразуване...';
                    case 'errored':
                        return 'Опитайте по-късно.';
                    case 'cancelled':
                        return 'Изтеглянето прекратено.';
                    case 'paused':
                        return 'Изтеглянето е на пауза.';
                }
            },
