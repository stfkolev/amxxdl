<template>
    <div class="home">
        ...
        <div v-if="loading" class="loading-screen">
            <h1>Извличане на най-новите видеоклипове, изчакайте...</h1>
            <folding-rectangles/>
        </div>
        ...
        <!-- Video cards -->
        <div v-if="channelIds.length > 1 && !loading" class="video-snippets">
            <h1>Вие стее абонирани към {{ channelIds.length }} канала</h1>
            ...
            refreshVideoCards() {
                ...
                ipcClient.request('getSettings').then(res => {
                    ...
                    this.$store.commit('dialogbox/show', {
                        type: 'error',
                        title: 'Опа',
                        text: 'Не можахме да извлечем видеата, опитайте по-късно.',
                        buttons: [
                            {type: 'accept', text: 'Добре'}
                        ]
                    });
                }).finally(() => {
                    this.loading = false;
