...
const downloads = {
    ...
    async startDownload({playlist, format, video, sources, downloadId = createUID()}, {sender}) {
        ...
        // Log
        await log('INFO', `Изтеглянето с id ${downloadId}: Започнато`);
        ...
        for (const {itag, container} of sources) {
            ...
            // Log
            await log('INFO', `Изтеглянето с id ${downloadId}: Започване на изтегляне с itag ${itag} / ${container}`);
            ...
            sourceStream.on('end', () => {
                ...
                if (done === sources.length) {
                    ...
                    encoder[sources.length === 1 ? 'convert' : 'merge'](sourceFiles, destinationFile).then(() => {
                        ...
                        // Log
                        log('ERROR', `Кодировка ${downloadId}: Грешка ${e.message} / ${e.toString()}`);
        ...
        activeDownloads[downloadId] = {
            cancel() {
                log('INFO', `Изтеглянето с id ${downloadId}: Прекъснато`);
            ...
            pause() {
                log('INFO', `Изтеглянето с id ${downloadId}: На пауза`);
                sourceStreams.forEach(s => !s.isPaused() && s.pause());
                update({status: 'paused'});
            },

            resume() {
                log('INFO', `Изтеглянето с id ${downloadId}: Продължено`);
                sourceStreams.forEach(s => s.isPaused() && s.resume());
                update({status: 'progress'});
            },

            retry() {
                log('INFO', `Изтеглянето с id ${downloadId}: Втори опит`);
